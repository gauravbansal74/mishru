...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', customizable = '" . $this->db->escape($value['customizable']) . "', region = " . $this->db->escape($value['region']).", region1 = '" . $this->db->escape($value['region1']) . "', region2 = '" . $this->db->escape($value['region2']) . "', region3 = '" . $this->db->escape($value['region3']) . "', region4 = '" . $this->db->escape($value['region4']) . "', region5 = '" . $this->db->escape($value['region5']) ."', region1_name = '" . $this->db->escape($value['region1_name']) . "', region2_name = '" . $this->db->escape($value['region2_name']) ."', region3_name = '" . $this->db->escape($value['region3_name']) ."', region4_name= '" . $this->db->escape($value['region4_name']) ."', region5_name = '" . $this->db->escape($value['region5_name']) ."'");
		}
	...
	public function getProductDescriptions($product_id) {
		...
		foreach ($query->rows as $result) {
			$product_description_data[$result['language_id']] = array(
				...
				'region1_name'     => $result['region1_name'],
				'region2_name'     => $result['region2_name'],
				'region3_name'     => $result['region3_name'],
				'region4_name'     => $result['region4_name'],
				'region5_name'     => $result['region5_name'],
