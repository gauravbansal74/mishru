 ...
 class ControllerModuleComboProducts extends Controller {
    ...
    public function install() {
        ...
        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "combo_setting` (
		  ...
		  `primary_product` int(11) NOT NULL AUTO_INCREMENT,
	...
	protected function getList (){
		...
		foreach ($results as $result) {
			...
			$primary_product_in_combo = $this->model_module_comboproducts->getPrimaryProduct($result['combo_id']);
			$primary_product_info = $this->model_catalog_product->getProductDescriptions($primary_product_in_combo);
			$primary_product_in_combo = $primary_product_info[$this->config->get('config_language_id')]['name'];
			
			$data['combos'][] = array(
				'combo_id' 				=> $result['combo_id'], 
				'combo_name'    		=> $result['combo_name'],
				'primary_product'		=> $primary_product_in_combo,
	...
	protected function getForm () {
		...
		if (isset($this->error['primary_product'])) {
			$data['error_primary_product'] = $this->error['primary_product'];
		} else {
			$data['error_primary_product'] = '';
		}		
		...
		if (isset($get_combo)) {
			...
			$primary_product = $get_combo['primary_product'];
	...
	protected function validateForm() {
		...
		if (!isset($this->request->post['primary_product'])) {
			$this->error['primary_product'] = $this->language->get('error_primary_product');
		} elseif (!is_numeric($this->request->post['primary_product'])) {
			$this->error['primary_product'] = $this->language->get('error_primary_product');
		}	
